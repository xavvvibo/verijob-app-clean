...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!user) {
    return json(res, 401, { error: "Unauthorized", route: "/pages/api/candidate/public-link" });
  }
  ...
  if (existing?.id) {
    ...
    if (error) {
      return json(res, 400, { error: "Update failed", route: "/pages/api/candidate/public-link" });
    }
  ...
  if (error) {
    return json(res, 400, { error: "Insert failed", route: "/pages/api/candidate/public-link" });
  }
