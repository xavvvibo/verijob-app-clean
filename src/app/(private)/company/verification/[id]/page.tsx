...
export default async function CompanyVerificationDetail({ params }: PageProps) {
  ...
  // Auth guard (el layout ya lo hace, pero aquí lo dejamos para endpoint consistency)
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link href="/company/requests" className="text-sm underline text-gray-600">
      ...
      <div className="border rounded-xl p-6 space-y-4 shadow-sm bg-white">
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="font-medium">{summary.company_name_freeform ?? "—"}</div>
          </div>

          <span className={`px-3 py-1 text-xs font-medium rounded-full ${badgeStyles(statusEffective)}`}>
      ...
      {summary.is_revoked && (
        <div className="border border-gray-300 bg-gray-50 rounded-xl p-4 space-y-2">
          <div className="font-semibold text-gray-800 text-sm">Verificación revocada</div>
      ...
      <div className="flex gap-4">
        <Link href={`/api/verification/${id}/summary`} className="text-sm underline text-blue-600">
          Ver JSON resumen
        </Link>
        <Link href={`/company/verification/${id}/evidences`} className="text-sm underline text-blue-600">
          Ver evidencias
        </Link>
      </div>
    </div>
