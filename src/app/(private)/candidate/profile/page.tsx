...
export default async function CandidateProfilePage() {
  ...
  const { data: profile } = await supabase
    .from("profiles")
    .select("id, full_name, phone, title, location, address_line1, address_line2, city, region, postal_code, country, identity_type, identity_masked")
    .eq("id", user.id)
    .maybeSingle();
    ...
    <div className="p-6 space-y-6">
      ...
      <CandidateProfileIdentityClient
        initialProfile={{
          ...
          has_identity: Boolean((profile as any)?.identity_masked || (profile as any)?.identity_type),
        }}
      />
    </div>
  );
